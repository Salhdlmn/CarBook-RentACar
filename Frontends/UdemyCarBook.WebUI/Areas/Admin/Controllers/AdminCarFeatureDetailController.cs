    ...
    public class AdminCarFeatureDetailController : Controller
    {
        ...
        public async Task<IActionResult> Index(List<ResultCarFeatureByCarIdDto> resultCarFeatureByCarIdDto)
        {
            ...
            foreach (var item in resultCarFeatureByCarIdDto)
            {
                ...
                {
                    var client = _httpClientFactory.CreateClient();
                    await client.GetAsync("https://localhost:7003/api/CarFeatures/CarFeatureChangeAvailableToTrue?id=" + item.CarFeatureID);

                }
                else
                {
                    var client = _httpClientFactory.CreateClient();
                    await client.GetAsync("https://localhost:7003/api/CarFeatures/CarFeatureChangeAvailableToFalse?id=" + item.CarFeatureID);
                }
            }
